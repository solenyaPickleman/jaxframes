...
class TestDistributedFrameIntegration:
    ...
    def _normalize(df: pd.DataFrame, sort_cols: list[str]) -> pd.DataFrame:
        """Sort and reset index for stable dataframe comparisons."""
    ...
    def test_to_pandas_decodes_string_columns(self):
        """Distributed frames should decode encoded string columns on conversion."""
        df = DistributedJaxFrame({
            'key': np.array(['b', 'a', None, 'b'], dtype=object),
            'value': jnp.array([1, 2, 3, 4]),
        })

        result = df.to_pandas()

        assert result['key'].tolist() == ['b', 'a', None, 'b']
        assert result['value'].tolist() == [1, 2, 3, 4]

    def test_frame_groupby_string_key(self):
        """Groupby should work on distributed string keys."""
        df = DistributedJaxFrame({
            'key': np.array(['b', 'a', 'b', None, 'a'], dtype=object),
            'value': jnp.array([10.0, 20.0, 30.0, 40.0, 50.0]),
        })

        result = df.groupby('key').sum().to_pandas()
        ...
        pd.testing.assert_frame_equal(
            self._normalize(result, ['key']),
            self._normalize(expected, ['key']),
            check_dtype=False,
        )

    def test_frame_groupby_multi_key_string_numeric(self):
        """Groupby should support mixed string and numeric keys."""
        df = DistributedJaxFrame({
            'key': np.array(['a', 'a', 'b', 'b', None], dtype=object),
            'bucket': jnp.array([1, 1, 1, 2, 2]),
            'value': jnp.array([10.0, 20.0, 30.0, 40.0, 50.0]),
        })

        result = df.groupby(['key', 'bucket']).sum().to_pandas()
        expected = pd.DataFrame({
            'key': ['a', 'b', 'b'],
            'bucket': [1, 1, 2],
            'value': [30.0, 30.0, 40.0],
        })

        pd.testing.assert_frame_equal(
            self._normalize(result, ['key', 'bucket']),
            self._normalize(expected, ['key', 'bucket']),
            check_dtype=False,
        )

    def test_frame_merge_string_key_with_string_payloads(self):
        """Merge should align string key vocabularies and preserve string payloads."""
        left_df = DistributedJaxFrame({
            'key': np.array(['b', 'a', None], dtype=object),
            'left_label': np.array(['left-b', 'left-a', 'left-null'], dtype=object),
            'left_value': jnp.array([1, 2, 3]),
        })
        right_df = DistributedJaxFrame({
            'key': np.array(['a', None, 'c'], dtype=object),
            'right_label': np.array(['right-a', 'right-null', 'right-c'], dtype=object),
            'right_value': jnp.array([10, 20, 30]),
        })

        result = left_df.merge(right_df, on='key', how='outer').to_pandas()
        expected = pd.DataFrame({
            'key': ['a', 'b', 'c', None],
            'left_left_label': ['left-a', 'left-b', None, 'left-null'],
            'left_left_value': [2.0, 1.0, np.nan, 3.0],
            'right_right_label': ['right-a', None, 'right-c', 'right-null'],
            'right_right_value': [10.0, np.nan, 30.0, 20.0],
        })

        pd.testing.assert_frame_equal(
            self._normalize(result, ['key', 'left_left_value', 'right_right_value']),
            self._normalize(expected, ['key', 'left_left_value', 'right_right_value']),
            check_dtype=False,
        )

    def test_frame_merge_multi_key_string_numeric(self):
        """Merge should support mixed string and numeric join keys."""
        left_df = DistributedJaxFrame({
            'key': np.array(['a', 'a', 'b'], dtype=object),
            'bucket': jnp.array([1, 2, 1]),
            'left_value': jnp.array([10, 20, 30]),
        })
        right_df = DistributedJaxFrame({
            'key': np.array(['a', 'b', 'b'], dtype=object),
            'bucket': jnp.array([2, 1, 2]),
            'right_label': np.array(['ra2', 'rb1', 'rb2'], dtype=object),
        })

        result = left_df.merge(right_df, on=['key', 'bucket'], how='outer').to_pandas()
        expected = pd.DataFrame({
            'key': ['a', 'a', 'b', 'b'],
            'bucket': [1, 2, 1, 2],
            'left_left_value': [10.0, 20.0, 30.0, np.nan],
            'right_right_label': [None, 'ra2', 'rb1', 'rb2'],
        })
...
class TestMultiDeviceAlgorithms:
    ...
    def test_distributed_string_groupby(self):
        """String-key groupby should work on actual sharded data."""
        devices = jax.devices()[:2]
        mesh = Mesh(devices, axis_names=('data',))
        sharding_spec = row_sharded(mesh)

        data = {
            'key': np.array(['a', 'b', 'a', 'b', 'c', None], dtype=object),
            'value': jnp.array([1, 2, 3, 4, 5, 6], dtype=jnp.float32),
        }
        df = DistributedJaxFrame(data, sharding=sharding_spec)

        result = df.groupby('key').sum().to_pandas().sort_values('key').reset_index(drop=True)
        expected = pd.DataFrame({
            'key': ['a', 'b', 'c'],
            'value': [4.0, 6.0, 5.0],
        })
